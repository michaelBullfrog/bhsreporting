...
def home():
    ...
    <body>
      <main class="shell">
        ...
        <div class="status">Dashboard backend v7.3</div>
      </main>
    </body>
    </html>
    """
...
function renderHours(rows){
  ...
  rows.forEach(r=>{
    if(r.date!==lastDate){
      ...
      html.push(`<tr><td colspan="7" style="text-align:left;background:#f4f8f5;font-weight:850;color:var(--brand-dark)">${label}</td></tr>`);
      lastDate=r.date;
    }
    html.push(`<tr>
...
.panel-body{padding:16px 18px}.trend{display:flex;align-items:end;gap:10px;height:220px;border-bottom:1px solid var(--line);padding:16px 6px 0}.barwrap{flex:1;min-width:34px;text-align:center}.bar{width:100%;background:#dbeee3;border-radius:8px 8px 0 0;position:relative;min-height:2px}.bar.missed{background:#f4c7c3}.barlabel{font-size:11px;color:var(--muted);margin-top:6px}.barvalue{font-size:11px;font-weight:800;margin-bottom:4px}
...
.panel-body{padding:16px 18px}.bars{display:block;padding:6px 0}.barwrap{flex:1;min-width:34px;text-align:center}.pair{display:flex;align-items:end;gap:3px;height:185px}.bar{flex:1;border-radius:6px 6px 0 0;min-height:2px}.bar.in{background:#cfe8d8}.bar.out{background:#9fcdb3}.barlabel{font-size:10px;color:var(--muted);margin-top:6px}.day-group{margin-bottom:18px}.day-title{font-size:13px;font-weight:850;margin:4px 0 8px;color:var(--ink)}.day-bars{display:flex;align-items:end;gap:9px;height:215px;border:1px solid var(--line);border-radius:12px;padding:12px 10px 0;background:#fbfdfc;overflow-x:auto}.date-cell{font-weight:800}.hour-sub{display:block;color:var(--muted);font-size:11px;font-weight:500;margin-top:2px}.barvalue{font-size:10px;color:var(--muted);margin-bottom:3px}
...
<main class="page">
  ...
  <section class="panel" style="margin-bottom:18px"><div class="panel-head"><div><div class="panel-title">Hourly Call Mix</div><div class="panel-meta">Grouped by date · inbound vs outbound task counts</div></div></div><div class="panel-body"><div id="bars" class="bars"></div></div></section>
  ...
  <section class="panel" style="margin-top:18px">
    <div class="panel-head"><div><div class="panel-title">Hourly Call Activity</div><div class="panel-meta">Inbound and outbound volume by date and hour</div></div></div>
    <div class="table-wrap"><table>
      <thead><tr><th>Hour</th><th>Inbound Calls</th><th>Queued Inbound</th><th>Outbound Calls</th><th>Outbound Share</th><th>Connected Outbound</th><th>Outbound Talk Time</th></tr></thead>
  ...
  Object.keys(byDate).sort().forEach(date=>{
    ...
    byDate[date].sort((a,b)=>a.hour-b.hour).forEach(r=>{
      ...
      w.innerHTML=`<div class="barvalue">${r.inbound_calls}/${r.outbound_calls}</div><div class="pair">
        <div class="bar in" style="height:${Math.max(2,r.inbound_calls/max*165)}px" title="Inbound ${r.inbound_calls}"></div>
        <div class="bar out" style="height:${Math.max(2,r.outbound_calls/max*165)}px" title="Outbound ${r.outbound_calls}"></div>
