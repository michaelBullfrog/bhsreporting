...
def missed_callbacks_summary(
    ...
    Manual outbound callback correlation is not yet implemented here.
    """
    try:
        tz = ZoneInfo(timezone_name)
    except ZoneInfoNotFoundError:
    ...
    callback_rows = [row for row in rows if callback_present(row)]
    successful_callbacks = [row for row in callback_rows if callback_success(row)]

    # Native callback timing uses callbackRequestTime -> callbackConnectTime.
    ...
    # Per-queue missed/callback summary.
    queue_buckets = {}
    for row in rows:
        queue_name = (row.queue_name or "Unknown Queue").strip()
        ...
        if callback_present(row):
            bucket["native_callbacks"] += 1
            if callback_success(row):
                bucket["successful_callbacks"] += 1

    queues = []
    for bucket in queue_buckets.values():
        qd = bucket["queued_inbound"]
        missed = bucket["missed"]
        callbacks = bucket["native_callbacks"]
        queues.append({
            **bucket,
            "missed_rate": round(missed / qd * 100, 2) if qd else 0,
            "callback_success_rate": (
                round(bucket["successful_callbacks"] / callbacks * 100, 2)
                if callbacks else 0
            ),
        })
    ...
    for row in rows:
        ...
        if callback_present(row):
            daily["native_callbacks"] += 1
            if callback_success(row):
                daily["successful_callbacks"] += 1
    ...
    for row in daily:
        ...
        row["callback_success_rate"] = (
            round(row["successful_callbacks"] / row["native_callbacks"] * 100, 2)
            if row["native_callbacks"] else 0
        )

    # Current unresolved approximation:
    # missed calls without successful native callback.
    unresolved_rows = [
        row for row in missed_rows
        if not callback_success(row)
    ]

    # Optional >24h style metric is based on end of selected window / current time.
    if to_ms is not None:
        reference_ms = to_ms
    else:
    ...
    return {
        ...
        "definitions": {
            ...
            "unresolved": "Missed interaction without a successful native callback",
        },
        "overview": {
            ...
            "native_callbacks": len(callback_rows),
            "successful_callbacks": len(successful_callbacks),
            "callback_success_rate": (
                round(len(successful_callbacks) / len(callback_rows) * 100, 2)
                if callback_rows else 0
            ),
            "avg_callback_seconds": round(avg_callback_ms / 1000, 2),
            "max_callback_seconds": round(max_callback_ms / 1000, 2),
