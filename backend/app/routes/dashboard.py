...
from ..services.metrics import overview, agent_summary, staffing_summary, call_demand_summary, service_sla_summary, missed_callbacks_summary
...
def get_missed_callbacks(
    from_ms: int | None = Query(None),
    to_ms: int | None = Query(None),
    unresolved_after_hours: int = Query(24, ge=0, le=720),
    ...
    result = missed_callbacks_summary(
        ...
        from_ms=from_ms,
        to_ms=to_ms,
        unresolved_after_hours=unresolved_after_hours,
...
def get_version():
    return {
        ...
        "staffing_route": "v6.3-fix-callback-diagnostics-scope",
    }
