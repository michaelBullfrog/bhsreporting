...
<section class="kpis"><div class="kpi"><div class="label">Voicemail Events</div><div class="value" id="total">—</div></div><div class="kpi"><div class="label">Unique Callers</div><div class="value" id="callers">—</div></div><div class="kpi"><div class="label">Successful Events</div><div class="value" id="success">—</div></div><div class="kpi"><div class="label">Avg CDR Duration</div><div class="value" id="avg">—</div></div><div class="kpi"><div class="label">Longest CDR Duration</div><div class="value" id="longest">—</div></div></section>
...
<section class="panel"><div class="panel-title">Voicemail Event Details</div><div class="panel-meta">Service After Hours Vmail only</div><div class="table-wrap"><table><thead><tr><th>Received</th><th>Caller</th><th>Caller Name</th><th>Duration</th><th>Outcome</th><th>Redirected From</th><th>Correlation ID</th></tr></thead><tbody id="rows"></tbody></table></div><div class="note">A “Voicemail Event” means Webex Calling generated a CDR leg for the Service After Hours voicemail group. Because this mailbox uses EXTERNAL message storage, CDR confirms that the call reached the voicemail group; it does not prove that the external email was opened/listened to, and CDR Duration is not guaranteed to equal the recorded audio length.</div></section>
</main><script>
...
async function loadData(){try{$('status').textContent='Loading voicemail data…';const f=startMs($('fromDate').value),t=endMs($('toDate').value);const r=await fetch(`/api/voicemail/service-after-hours?from_ms=${f}&to_ms=${t}&timezone=America/Detroit`);if(!r.ok)throw new Error(await r.text());const d=await r.json(),s=d.summary||{};$('total').textContent=s.voicemail_events||0;$('callers').textContent=s.unique_callers||0;$('success').textContent=s.successful_events||0;$('avg').textContent=dur(s.avg_duration_seconds);$('longest').textContent=dur(s.longest_duration_seconds);bars('daily',d.daily||[],'date');bars('hourly',d.hourly||[],'hour');$('rows').innerHTML=(d.details||[]).map(x=>`<tr><td>${dt(x.received_time)}</td><td>${phone(x.caller_number)}</td><td>${x.caller_name||'—'}</td><td>${dur(x.duration_seconds)}</td><td><span class="pill">${x.outcome||'—'}</span></td><td>${phone(x.redirecting_number)}</td><td title="${x.correlation_id||''}">${String(x.correlation_id||'—').slice(0,12)}…</td></tr>`).join('')||'<tr><td colspan="7">No Service After Hours voicemail events in this period.</td></tr>';$('status').textContent=`Data source: PostgreSQL · Backend ${d.backend_version} · ${d.voicemail_group?.name||''}`;}catch(e){$('status').textContent='Voicemail data unavailable: '+e.message}}
