...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .top-link { color:#d8eadf; text-decoration:none; font-size:13px; }.nav{display:flex;gap:15px;flex-wrap:wrap;align-items:center}.nav a{color:#cde3d5;text-decoration:none;font-size:13px}.nav a.active{color:#fff;font-weight:800}
...
<body>
  <header class="topbar">
    ...
    <nav class="nav"><a href="/executive-overview">Executive</a><a class="active" href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a></nav>
  </header>
...
<header class="topbar">
  ...
  <nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a class="active" href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a></nav>
</header>
...
<header class="topbar">
  ...
  <nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a class="active" href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a></nav>
</header>
...
function renderHours(rows){
  ...
  rows.forEach(r=>{
    if(r.date!==lastDate){
      const label=new Date(r.date+'T00:00:00').toLocaleDateString(undefined,{weekday:'long',month:'short',day:'numeric',year:'numeric'});
      html.push(`<tr><td colspan="7" style="text-align:left;background:#f4f8f5;font-weight:850;color:var(--brand-dark);padding:7px 12px">${label}</td></tr>`);
      lastDate=r.date;
    }
    html.push(`<tr>
      <td><span class="date-cell">${r.hour_label}</span></td>
      <td>${r.inbound_calls}</td><td>${r.queued_inbound}</td><td>${r.outbound_calls}</td>
      <td>${pct(r.outbound_share)}</td><td>${r.outbound_connected}</td><td>${duration(r.outbound_talk_seconds)}</td>
    </tr>`);
  });
  $('hourRows').innerHTML=html.join('')||'<tr><td colspan="7" class="empty">No hourly activity.</td></tr>';
}
...
<div class="topbar">
  ...
  <nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a class="active" href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a></nav>
</div>
<main class="page">
  <div class="heading-row">
...
.panel-body{padding:16px 18px}.bars{display:block;padding:6px 0}.barwrap{flex:1;min-width:26px;text-align:center}.pair{display:flex;align-items:end;gap:2px;height:92px}.bar{flex:1;border-radius:6px 6px 0 0;min-height:2px}.bar.in{background:#cfe8d8}.bar.out{background:#9fcdb3}.barlabel{font-size:9px;color:var(--muted);margin-top:4px}.day-group{margin-bottom:10px}.day-title{font-size:12px;font-weight:850;margin:2px 0 5px;color:var(--ink)}.day-bars{display:flex;align-items:end;gap:5px;height:125px;border:1px solid var(--line);border-radius:10px;padding:8px 8px 3px;background:#fbfdfc;overflow-x:auto}.date-cell{font-weight:800}.hour-sub{display:block;color:var(--muted);font-size:11px;font-weight:500;margin-top:2px}.barvalue{font-size:9px;color:var(--muted);margin-bottom:2px}
...
.note{font-size:12px;line-height:1.55;color:var(--muted);padding:14px 18px;border-top:1px solid var(--line)}.heatmap-wrap{overflow-x:auto;padding:14px 18px 18px}.heatmap{display:grid;gap:4px;min-width:900px;align-items:stretch}.heat-head{font-size:10px;font-weight:800;color:var(--muted);text-align:center;padding:5px 2px}.heat-date{font-size:11px;font-weight:800;color:var(--ink);display:flex;align-items:center;padding-right:8px;white-space:nowrap}.heat-cell{min-height:48px;border:1px solid var(--line);border-radius:7px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:4px 3px;font-size:10px;line-height:1.2;cursor:default}.heat-cell strong{font-size:12px}.heat-cell span{color:var(--muted);font-size:9px;margin-top:2px}.heat-empty{background:#fafcfb;color:#a9b3ae}.heat-1{background:#edf7f1}.heat-2{background:#dceee3}.heat-3{background:#c3e1cf}.heat-4{background:#9dceb1}.heat-5{background:#6eb48d;color:#102a1c}.heat-5 span{color:#214d35}.heat-legend{display:flex;align-items:center;gap:6px;flex-wrap:wrap;font-size:11px;color:var(--muted);padding:0 18px 14px}.legend-box{width:18px;height:14px;border:1px solid var(--line);border-radius:4px}.legend-label{margin-right:4px}.empty{color:var(--muted);padding:22px;text-align:center}.error{padding:20px;color:var(--bad)}
.modal{display:none;position:fixed;inset:0;background:rgba(15,30,22,.45);z-index:20;align-items:center;justify-content:center;padding:20px}.modal.open{display:flex}.modal-card{width:min(760px,100%);max-height:85vh;overflow:auto;background:#fff;border-radius:18px;padding:22px}.modal-head{display:flex;justify-content:space-between;gap:12px}.close{border:0;background:none;font-size:24px;cursor:pointer}.def{padding:12px 0;border-bottom:1px solid var(--line)}.def strong{display:block;margin-bottom:4px}.def p{margin:0;color:var(--muted);font-size:13px;line-height:1.5}
@media(max-width:1100px){.kpis{grid-template-columns:repeat(4,1fr)}.grid{grid-template-columns:1fr}}@media(max-width:700px){.kpis{grid-template-columns:repeat(2,1fr)}.filters{grid-template-columns:1fr}.page{padding:18px}.nav{display:none}}
</style>
</head>
...
<div class="topbar">
  <div class="brand"><div class="mark">B</div><div><div class="brand-title">Bullfrog Analytics</div><div class="brand-sub">Webex Contact Center</div></div></div>
  <nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a class="active" href="/inbound-outbound">Call Activity</a></nav>
</div>
<main class="page">
  <div class="heading-row">
  ...
  <section class="panel" style="margin-bottom:18px"><div class="panel-head"><div><div class="panel-title">Hourly Call Mix</div><div class="panel-meta">Grouped by date · light = inbound · dark = outbound</div></div></div><div class="panel-body"><div id="bars" class="bars"></div></div></section>
...
function renderBars(rows){
  ...
  Object.keys(byDate).sort().forEach(date=>{
    ...
    const label=new Date(date+'T00:00:00').toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric',year:'numeric'});
    ...
    byDate[date].sort((a,b)=>a.hour-b.hour).forEach(r=>{
      ...
      w.innerHTML=`<div class="barvalue">${r.inbound_calls}/${r.outbound_calls}</div><div class="pair">
        <div class="bar in" style="height:${Math.max(2,r.inbound_calls/max*78)}px" title="Inbound ${r.inbound_calls}"></div>
        <div class="bar out" style="height:${Math.max(2,r.outbound_calls/max*78)}px" title="Outbound ${r.outbound_calls}"></div>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Executive Overview | WxCC Analytics</title>
<style>
:root{--bg:#f3f6f5;--panel:#fff;--ink:#17221d;--muted:#69766f;--line:#dde6e1;--brand:#1f7a4c;--brand-dark:#145c38;--soft:#e9f5ee;--bad:#b42318;--badbg:#fff0ee;--warn:#9a6700;--warnbg:#fff7dd;--shadow:0 7px 24px rgba(20,55,39,.07)}
*{box-sizing:border-box}body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--ink)}
button,input{font:inherit}.topbar{height:66px;display:flex;align-items:center;justify-content:space-between;padding:0 28px;background:#173d2b;color:#fff;position:sticky;top:0;z-index:10}
.brand{display:flex;gap:12px;align-items:center}.mark{width:34px;height:34px;display:grid;place-items:center;border-radius:10px;background:#2b8f5d;font-weight:900}.brand-title{font-weight:800}.brand-sub{font-size:12px;color:#bbd7c7}
.nav{display:flex;gap:15px;flex-wrap:wrap}.nav a{color:#cde3d5;text-decoration:none;font-size:13px}.nav a.active{color:#fff;font-weight:800}
.page{max-width:1500px;margin:auto;padding:28px}.heading-row{display:flex;justify-content:space-between;align-items:flex-end;gap:18px;flex-wrap:wrap;margin-bottom:20px}
h1{margin:0;font-size:30px}.subtitle{margin-top:5px;color:var(--muted);font-size:14px}.policy{margin-top:6px;color:var(--muted);font-size:12px}
.filters{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:16px;display:grid;grid-template-columns:minmax(160px,1fr) minmax(160px,1fr) auto;gap:12px;align-items:end;box-shadow:var(--shadow);margin-bottom:18px}
...
.kpis{display:grid;grid-template-columns:repeat(8,minmax(120px,1fr));gap:12px;margin-bottom:18px}.kpi{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:var(--shadow)}.kpi-label{font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);font-weight:800}.kpi-value{font-size:28px;font-weight:850;margin-top:8px}.kpi-foot{font-size:11px;color:var(--muted);margin-top:3px}
...
<div class="topbar">
  <div class="brand"><div class="mark">B</div><div><div class="brand-title">Bullfrog Analytics</div><div class="brand-sub">Webex Contact Center</div></div></div>
  <nav class="nav"><a class="active" href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a></nav>
</div>
<main class="page">
  <div class="heading-row">
    <div><h1>Executive Overview</h1><div class="subtitle">A management-level snapshot of demand, service, staffing, missed calls, and call activity.</div><div class="policy">15-second SLA · 5-minute long-wait threshold · factual attention items only</div></div>
    <button class="btn" style="background:#fff;color:var(--brand-dark);border:1px solid var(--line)" onclick="openDefs()">Metric Definitions</button>
  </div>
...
function localStart(v){if(!v)return null;return new Date(v+'T00:00:00').getTime()}
function localAfter(v){if(!v)return null;const d=new Date(v+'T00:00:00');d.setDate(d.getDate()+1);return d.getTime()}
function setDefaults(){const now=new Date(),from=new Date(now);from.setDate(now.getDate()-7);$('toDate').value=now.toISOString().slice(0,10);$('fromDate').value=from.toISOString().slice(0,10)}
