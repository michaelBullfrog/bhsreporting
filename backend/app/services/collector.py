...
def collect_window(db: Session, from_ms: int, to_ms: int) -> dict:
    ...
    try:
        ...
        for session in agent_sessions:
            ...
            telephony = next((c for c in (session.get("channelInfo") or []) if (c.get("channelType") or "").lower() == "telephony"), {})
            ...
            _upsert(db, AgentSession, "agent_session_id", session_id, session_values)
            ...
            for channel in session.get("channelInfo") or []:
                ...
                for activity in activities:
                    ...
                    duration_ms = None
                    if isinstance(start, int) and isinstance(end, int) and end >= start and end != -1:
                        duration_ms = end - start

                    raw_state = activity.get("state")
                    activity_lower = activity_id.lower()

                    # Preserve ID-derived detail without reclassifying the entire
                    # idle interval as RONA. A true RONA duration is represented
                    # by the explicit "not-responding" state.
                    state_detail = None
                    ...
                    _upsert(db, AgentStateActivity, "activity_id", activity_id, activity_values)
