...
from .routes.voicemail import router as voicemail_router
...
<body>
  <header class="topbar">
    ...
    <nav class="nav"><a href="/executive-overview">Executive</a><a class="active" href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a><a href="/voicemail">Voicemail</a><a href="/auth/logout">Sign out</a></nav><div class="bhs-logo-wrap"><img class="bhs-header-logo" src="/static/bhs_logo.png" alt="BHS Corrugated logo"></div>
  </header>
...
<header class="topbar">
  ...
  <nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a class="active" href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a><a href="/voicemail">Voicemail</a><a href="/auth/logout">Sign out</a></nav><div class="bhs-logo-wrap"><img class="bhs-header-logo" src="/static/bhs_logo.png" alt="BHS Corrugated logo"></div>
</header>
...
<header class="topbar">
  <div class="brand"><div><div class="brand-title">BHS Corrugated</div><div class="brand-sub">Contact Center Analytics</div></div></div>
  <nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a class="active" href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a><a href="/voicemail">Voicemail</a><a href="/auth/logout">Sign out</a></nav><div class="bhs-logo-wrap"><img class="bhs-header-logo" src="/static/bhs_logo.png" alt="BHS Corrugated logo"></div>
</header>
...
.callback-activity-summary{display:flex;gap:8px;align-items:center;flex-wrap:wrap}
.callback-stat{display:inline-flex;gap:5px;align-items:center;padding:6px 9px;border-radius:999px;background:#eef5fa;color:#5f7485;font-size:11px}
.callback-stat strong{color:#173247}
.callback-stat.success{background:#edf8f1;color:#397453}
.callback-stat.failure{background:#fff0ee;color:#9b3b34}
.status-pill{display:inline-flex;align-items:center;padding:4px 8px;border-radius:999px;font-size:10px;font-weight:850;letter-spacing:.02em}
.status-pill.success{background:#eaf7ef;color:#247246}
.status-pill.failure{background:#fff0ee;color:#a43a32}
.status-pill.other{background:#eef4f8;color:#5f7485}
.match-pill{display:inline-flex;align-items:center;padding:4px 8px;border-radius:999px;font-size:10px;font-weight:800}
.match-pill.yes{background:#eaf5fc;color:#0a649f}
.match-pill.no{background:#f1f4f6;color:#687b8a}
...
<div class="topbar">
  <div class="brand"><div><div class="brand-title">BHS Corrugated</div><div class="brand-sub">Contact Center Analytics</div></div></div>
  <nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a class="active" href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a><a href="/voicemail">Voicemail</a><a href="/auth/logout">Sign out</a></nav><div class="bhs-logo-wrap"><img class="bhs-header-logo" src="/static/bhs_logo.png" alt="BHS Corrugated logo"></div>
</div>
<main class="page">
  ...
  <section class="panel" style="margin-top:18px">
    <div class="panel-head">
      <div>
        <div class="panel-title">Native Callback Activity</div>
        <div class="panel-meta">All stored WxCC native callback records in the selected reporting period · Success and Failure</div>
      </div>
      <div class="callback-activity-summary">
        <span class="callback-stat"><strong id="nativeCallbackTotal">—</strong> Total</span>
        <span class="callback-stat success"><strong id="nativeCallbackSuccess">—</strong> Success</span>
        <span class="callback-stat failure"><strong id="nativeCallbackFailure">—</strong> Failure</span>
      </div>
    </div>
    <div class="table-wrap"><table>
      <thead><tr>
        ...
        <th>Connect Delay</th>
        <th>Callback Agent</th>
        <th>Team</th>
        <th>Status</th>
        <th>Retries</th>
        <th>Matched to Missed Call</th>
      </tr></thead>
      <tbody id="nativeCallbackRows"></tbody>
    </table></div>
    <div class="note">This table is the callback-event view. A native callback appears here even if it cannot be confidently correlated to a missed call. Final Reason / Terminated By are not currently stored by the validated GraphQL collector.</div>
  </section>

  <section class="panel" style="margin-top:18px">
    <div class="panel-head"><div><div class="panel-title">Missed Call Resolution</div><div class="panel-meta">Strict one-to-one successful native callback matches</div></div></div>
    <div class="table-wrap"><table>
      <thead><tr><th>Queue</th><th>Caller</th><th>Callback Agent</th><th>Team</th><th>Time From Missed Call to Callback</th></tr></thead>
      <tbody id="callbackRows"></tbody>
    </table></div>
    <div class="note">This table is intentionally stricter than Native Callback Activity. It only includes successful callbacks confidently matched to one missed call under the 24-hour one-to-one policy.</div>
  </section>
</main>
...
<div id="defs" class="modal" onclick="if(event.target===this)closeDefs()"><div class="modal-card">
  ...
  <div class="def"><strong>Native Callback Activity</strong><p>Every stored WxCC native callback record in the selected reporting period, regardless of Success/Failure or missed-call correlation.</p></div>
  <div class="def"><strong>Matched to Missed Call</strong><p>Yes when the callback is the unique successful callback used by the strict one-to-one missed-call correlation logic.</p></div>
...
function renderNativeCallbackActivity(rows,o){
  const total=(rows||[]).length;
  $('nativeCallbackTotal').textContent=total;
  $('nativeCallbackSuccess').textContent=Number(o?.successful_native_callback_records||0);
  $('nativeCallbackFailure').textContent=Number(o?.failed_native_callback_records||0);

  $('nativeCallbackRows').innerHTML=(rows||[]).map(r=>{
    const matched=!!r.matched_to_missed_call;
    const status=r.callback_status||'Unknown';
    return `<tr>
      <td>${r.queue_name||'—'}</td>
      <td>${phone(r.callback_number)}</td>
      <td>${r.callback_type||'—'}</td>
      <td>${r.callback_origin||'—'}</td>
      <td>${dateTime(r.callback_request_time)}</td>
      <td>${dateTime(r.callback_connect_time)}</td>
      <td>${r.callback_connect_delay_seconds===null||r.callback_connect_delay_seconds===undefined?'—':duration(r.callback_connect_delay_seconds)}</td>
      <td>${r.callback_agent_name||'—'}</td>
      <td>${r.callback_team_name||'—'}</td>
      <td><span class="status-pill ${statusClass(status)}">${status}</span></td>
      <td>${Number(r.callback_retry_count||0)}</td>
      <td><span class="match-pill ${matched?'yes':'no'}">${matched?'Yes':'No'}</span></td>
    </tr>`;
  }).join('')||'<tr><td colspan="12" class="empty">No native callback records in the selected period.</td></tr>';
}
...
async function loadData(){
  try{
    ...
    renderTrend(d.daily||[]);renderSummary(o);renderQueues(d.queues||[]);renderNativeCallbackActivity(d.native_callback_activity||[],o);renderMatches(d.resolved_matches||[]);
...
<div class="topbar">
  <div class="brand"><div><div class="brand-title">BHS Corrugated</div><div class="brand-sub">Contact Center Analytics</div></div></div>
  <nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a class="active" href="/inbound-outbound">Call Activity</a><a href="/voicemail">Voicemail</a><a href="/auth/logout">Sign out</a></nav><div class="bhs-logo-wrap"><img class="bhs-header-logo" src="/static/bhs_logo.png" alt="BHS Corrugated logo"></div>
</div>
<main class="page">
  <section class="health-strip">
    <div class="health-left">
...
<div class="topbar">
  <div class="brand"><div><div class="brand-title">BHS Corrugated</div><div class="brand-sub">Contact Center Analytics</div></div></div>
  <nav class="nav"><a class="active" href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a><a href="/voicemail">Voicemail</a><a href="/auth/logout">Sign out</a></nav><div class="bhs-logo-wrap"><img class="bhs-header-logo" src="/static/bhs_logo.png" alt="BHS Corrugated logo"></div>
</div>
<main class="page">
  <section class="health-strip">
    <div class="health-left">
...
<title>BHS Corrugated | Voicemail</title>
<style>
:root{--bg:#f4f7fa;--panel:#fff;--ink:#173247;--muted:#6a7d8d;--line:#dbe5ec;--navy:#07395f;--blue:#0b6fb8;--soft:#eaf4fb;--good:#247246;--warn:#a66a00;--bad:#9b2c2c}
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif}.topbar{background:var(--navy);color:#fff;padding:14px 26px;display:flex;justify-content:space-between;align-items:center;gap:20px;position:sticky;top:0;z-index:10}.brand{font-weight:850}.brand small{display:block;color:#bfd3e2;font-weight:500;margin-top:2px}.nav{display:flex;gap:15px;align-items:center;flex-wrap:wrap}.nav a{color:#d3e4f2;text-decoration:none;font-size:13px}.nav a.active{color:#fff;font-weight:850}.bhs-logo-wrap{background:#fff;border-radius:8px;padding:5px 9px}.bhs-header-logo{height:28px;display:block}.page{max-width:1500px;margin:auto;padding:28px}.head{display:flex;justify-content:space-between;gap:18px;align-items:flex-end;flex-wrap:wrap;margin-bottom:18px}h1{margin:0;font-size:30px}.sub{color:var(--muted);font-size:14px;margin-top:5px}.filters{display:flex;gap:10px;align-items:end;flex-wrap:wrap}.field{display:flex;flex-direction:column;gap:5px;font-size:11px;font-weight:750;color:#62788a}.field input{padding:9px 10px;border:1px solid var(--line);border-radius:9px;background:#fff}.btn{padding:10px 14px;border:0;border-radius:9px;background:var(--blue);color:#fff;font-weight:800;cursor:pointer}.kpis{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin:18px 0}.kpi{background:#fff;border:1px solid var(--line);border-radius:14px;padding:16px;box-shadow:0 5px 18px rgba(7,57,95,.04)}.kpi .label{font-size:11px;color:var(--muted);font-weight:800;text-transform:uppercase;letter-spacing:.04em}.kpi .value{font-size:27px;font-weight:900;margin-top:5px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.panel{background:#fff;border:1px solid var(--line);border-radius:14px;padding:16px;margin-bottom:14px}.panel-title{font-weight:900}.panel-meta{font-size:12px;color:var(--muted);margin-top:3px}.bars{display:flex;align-items:end;gap:5px;height:170px;margin-top:16px}.bar-wrap{flex:1;min-width:0;text-align:center}.bar{background:#5b98c4;border-radius:5px 5px 2px 2px;min-height:2px}.bar-label{font-size:9px;color:var(--muted);margin-top:5px}.table-wrap{overflow:auto;margin-top:12px}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#f5f8fa;color:#5f7485;text-align:left;padding:9px;border-bottom:1px solid var(--line);white-space:nowrap}td{padding:9px;border-bottom:1px solid #edf2f5;white-space:nowrap}.pill{display:inline-block;padding:4px 7px;border-radius:999px;background:#eaf7ef;color:var(--good);font-weight:800}.pill.attempt{background:#fff4d8;color:var(--warn)}.pill.outstanding{background:#fdecec;color:var(--bad)}.note{font-size:11px;color:var(--muted);margin-top:10px;line-height:1.5}.status{font-size:12px;color:var(--muted);margin:4px 0 14px}@media(max-width:1000px){.kpis{grid-template-columns:repeat(2,1fr)}.grid{grid-template-columns:1fr}}
</style></head><body>
<div class="topbar"><div><div class="brand">BHS Corrugated<small>Contact Center Analytics</small></div><nav class="nav"><a href="/executive-overview">Executive</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callbacks</a><a href="/inbound-outbound">Call Activity</a><a class="active" href="/voicemail">Voicemail</a><a href="/auth/logout">Sign out</a></nav></div><div class="bhs-logo-wrap"><img class="bhs-header-logo" src="/static/bhs_logo.png" alt="BHS Corrugated logo"></div></div>
<main class="page"><div class="head"><div><h1>Service After Hours Voicemail</h1><div class="sub">Webex Calling Detailed Call History · Service After Hours Vmail · Knoxville TN · ext 6005</div></div><div class="filters"><label class="field">From<input id="fromDate" type="date"></label><label class="field">To<input id="toDate" type="date"></label><button class="btn" onclick="loadData()">Apply</button></div></div>
<div id="status" class="status">Loading voicemail data…</div>
<section class="kpis"><div class="kpi"><div class="label">Voicemail Events</div><div class="value" id="total">—</div></div><div class="kpi"><div class="label">Called Back</div><div class="value" id="calledBack">—</div></div><div class="kpi"><div class="label">Callback Rate</div><div class="value" id="callbackRate">—</div></div><div class="kpi"><div class="label">Outstanding</div><div class="value" id="outstanding">—</div></div><div class="kpi"><div class="label">Avg Time to Callback</div><div class="value" id="avgCallback">—</div></div></section>
<div class="grid"><section class="panel"><div class="panel-title">Voicemail Events by Day</div><div class="panel-meta">One unique VoiceMailGroup event per Correlation ID</div><div id="daily" class="bars"></div></section><section class="panel"><div class="panel-title">Voicemail Events by Hour</div><div class="panel-meta">Local reporting time · America/Detroit</div><div id="hourly" class="bars"></div></section></div>
<section class="panel"><div class="panel-title">Voicemail Event Details</div><div class="panel-meta">Service After Hours Vmail only · callback match window: 72 hours</div><div class="table-wrap"><table><thead><tr><th>Received</th><th>Caller</th><th>Caller Name</th><th>Call Duration</th><th>Callback Status</th><th>Called Back By</th><th>Callback Time</th><th>Time to Callback</th><th>Callback Duration</th></tr></thead><tbody id="rows"></tbody></table></div><div class="note">A “Voicemail Event” means Webex Calling generated a CDR leg for the Service After Hours voicemail group. Because this mailbox uses EXTERNAL message storage, CDR confirms that the call reached the voicemail group; it does not prove that the external email was opened/listened to, and CDR Duration is not guaranteed to equal the recorded audio length.</div></section>
</main><script>
const $=id=>document.getElementById(id);function pad(n){return String(n).padStart(2,'0')}function localDate(d){return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`}function startMs(v){const [y,m,d]=v.split('-').map(Number);return new Date(y,m-1,d,0,0,0,0).getTime()}function endMs(v){const [y,m,d]=v.split('-').map(Number);return new Date(y,m-1,d+1,0,0,0,0).getTime()}function dur(s){s=Math.round(Number(s)||0);if(s<60)return `${s}s`;return `${Math.floor(s/60)}m ${s%60}s`}function phone(v){if(!v)return '—';const d=String(v).replace(/\\D/g,'');return d.length>=10?`***-${d.slice(-4)}`:v}function dt(ms){return ms?new Date(Number(ms)).toLocaleString([],{month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'}):'—'}function durLong(s){s=Math.round(Number(s)||0);if(s<60)return `${s}s`;if(s<3600)return `${Math.floor(s/60)}m`;const h=Math.floor(s/3600),m=Math.floor((s%3600)/60);return `${h}h ${m}m`}
function bars(id,items,labelKey){const max=Math.max(1,...items.map(x=>Number(x.count||0)));$(id).innerHTML=items.map(x=>`<div class="bar-wrap" title="${x[labelKey]}: ${x.count}"><div class="bar" style="height:${Math.max(2,Math.round((x.count/max)*135))}px"></div><div class="bar-label">${labelKey==='hour'?String(x.hour).padStart(2,'0'):String(x.date).slice(5)}</div></div>`).join('')}
async function loadData(){try{$('status').textContent='Loading voicemail data…';const f=startMs($('fromDate').value),t=endMs($('toDate').value);const r=await fetch(`/api/voicemail/service-after-hours?from_ms=${f}&to_ms=${t}&timezone=America/Detroit`);if(!r.ok)throw new Error(await r.text());const d=await r.json(),s=d.summary||{};$('total').textContent=s.voicemail_events||0;$('calledBack').textContent=s.called_back||0;$('callbackRate').textContent=`${Number(s.callback_rate_percent||0).toFixed(1)}%`;$('outstanding').textContent=s.outstanding||0;$('avgCallback').textContent=durLong(s.avg_callback_seconds);bars('daily',d.daily||[],'date');bars('hourly',d.hourly||[],'hour');$('rows').innerHTML=(d.details||[]).map(x=>{const cls=x.callback_status==='Callback Attempted'?'attempt':x.callback_status==='Outstanding'?'outstanding':'';return `<tr><td>${dt(x.received_time)}</td><td>${phone(x.caller_number)}</td><td>${x.caller_name||'—'}</td><td>${dur(x.duration_seconds)}</td><td><span class="pill ${cls}">${x.callback_status||'Outstanding'}</span></td><td>${x.callback_by||'—'}</td><td>${dt(x.callback_time)}</td><td>${x.callback_delay_seconds==null?'—':durLong(x.callback_delay_seconds)}</td><td>${x.callback_duration_seconds==null?'—':dur(x.callback_duration_seconds)}</td></tr>`}).join('')||'<tr><td colspan="9">No Service After Hours voicemail events in this period.</td></tr>';$('status').textContent=`Data source: PostgreSQL · Backend ${d.backend_version} · ${d.voicemail_group?.name||''}`;}catch(e){$('status').textContent='Voicemail data unavailable: '+e.message}}
const today=new Date(),from=new Date(today.getFullYear(),today.getMonth(),today.getDate()-6);$('fromDate').value=localDate(from);$('toDate').value=localDate(today);loadData();
</script></body></html>
"""
