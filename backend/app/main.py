...
def home():
    ...
    <body>
      <main class="shell">
        ...
        <div class="status">Dashboard backend v5.3</div>
      </main>
    </body>
    </html>
    """
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--ink)}
    ...
    .kpis{display:grid;grid-template-columns:repeat(9,minmax(115px,1fr));gap:12px;margin-bottom:18px}.kpi{background:var(--panel);border:1px solid var(--line);border-radius:15px;padding:17px;box-shadow:var(--shadow);min-width:0}
...
<main class="page" id="page">
  ...
  <section class="kpis">
    ...
    <div class="kpi"><div class="kpi-label">Answered Over SLA</div><div class="kpi-value" id="overSla">—</div><div class="kpi-foot">answered after 15s</div></div>
    <div class="kpi"><div class="kpi-label">Other Outcomes</div><div class="kpi-value" id="otherOutcomes">—</div><div class="kpi-foot">neither answered nor abandoned</div></div>
  </section>
  ...
  <section class="panel">
    ...
    <div class="queue-wrap">
      <table>
        <thead><tr>
          ...
          <th data-sort="answered">Answered</th>
          <th data-sort="abandoned">Abandoned</th>
          <th data-sort="other_outcomes">Other</th>
    ...
    <div class="note">Policy thresholds are fixed in this dashboard: 15-second SLA, 300-second long-wait threshold, and no short-abandon exclusion.</div>
  </section>
</main>
...
<div class="modal-backdrop" id="modal">
  <div class="modal-card">
    ...
    <div class="defs">
      ...
      <div class="def"><strong>Long-Wait Breach</strong><p>A queued interaction whose queue wait exceeds the fixed 300-second (5-minute) threshold.</p></div><div class="def"><strong>Other Outcome</strong><p>A queued interaction that was neither answered nor terminated as abandoned. These calls remain in the queued/SLA denominator and are shown separately so totals reconcile.</p></div>
...
function renderOverview(o){
  ...
  $('otherOutcomes').textContent=o.other_outcomes;
}
...
function renderInsights(o){
  const items=[
    ...
    ['Other Queue Outcomes',`${o.other_outcomes} calls were neither answered nor abandoned`],
  ];
...
function renderQueues(){
  ...
  $('queueBody').innerHTML=rows.map(r=>{
    ...
    return `<tr>
      <td class="queue-name"><span class="pill ${cls}">${esc(r.queue_name)}</span></td>
      <td>${r.queued_inbound}</td><td>${r.answered}</td><td>${r.abandoned}</td><td>${r.other_outcomes}</td>
