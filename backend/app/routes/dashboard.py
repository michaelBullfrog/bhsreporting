...
from ..services.metrics import overview, agent_summary, staffing_summary, call_demand_summary, service_sla_summary
...
def get_service_sla(
    from_ms: int | None = Query(None),
    to_ms: int | None = Query(None),
    sla_seconds: int = Query(60, ge=0, le=86400),
    long_wait_seconds: int = Query(300, ge=0, le=86400),
    short_abandon_seconds: int = Query(0, ge=0, le=86400),
...
def get_version():
    return {
        ...
        "staffing_route": "v5.1-service-sla-dashboard",
    }
