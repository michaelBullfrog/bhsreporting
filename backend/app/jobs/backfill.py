...
def main():
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Re-pull windows that already have a successful collector run.",
    )
    parser.add_argument(
        "--calls-only",
        action="store_true",
        help=(
            "Import task/taskDetails/taskLegDetails only. Recommended for "
            "large historical call backfills because old agent-session "
            "pagination can vary by WxCC schema/retention."
        ),
    )
            ...
            for w_from, w_to in windows:
                if not args.force:
                    existing = (
                        ...
                        .filter(
                            CollectorRun.from_ms == w_from,
                            CollectorRun.to_ms == w_to,
                            CollectorRun.success.is_(True),
                        )
                        .first()
                    )
                    if existing:
                        print(f"  SKIP {w_from} -> {w_to}: already successfully collected")
                        continue
                ...
                result = collect_window(
                    ...
                    w_from,
                    w_to,
                    include_agent_sessions=not args.calls_only,
                )
                ...
                print(
                    ...
                    f"{result['agent_sessions']} agent sessions"
                )
                if result.get("agent_session_error"):
                    print(
                        "  WARN agent-session history skipped for this window: "
                        f"{result['agent_session_error']}"
                    )
