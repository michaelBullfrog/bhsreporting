...
function setDefaults(){
  const now=new Date(), from=new Date(now);from.setDate(now.getDate()-29);
...
function localStart(v){if(!v)return null;return new Date(v+'T00:00:00').getTime()}
function localAfter(v){if(!v)return null;const d=new Date(v+'T00:00:00');d.setDate(d.getDate()+1);return d.getTime()}
function setDefaults(){const now=new Date(),from=new Date(now);from.setDate(now.getDate()-29);$('toDate').value=now.toISOString().slice(0,10);$('fromDate').value=from.toISOString().slice(0,10)}
