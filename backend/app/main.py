...
def home():
    ...
    <body>
      <main class="shell">
        ...
        <div class="status">Dashboard backend v5.1</div>
      </main>
    </body>
    </html>
    """
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--ink)}
    ...
    h1{margin:0;font-size:30px;letter-spacing:-.025em}.subtitle{margin-top:5px;color:var(--muted);font-size:14px}
    .filters{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:16px;display:grid;grid-template-columns:repeat(5,minmax(130px,1fr)) auto;gap:12px;align-items:end;box-shadow:var(--shadow);margin-bottom:18px}
    ...
    .kpis{display:grid;grid-template-columns:repeat(8,minmax(120px,1fr));gap:12px;margin-bottom:18px}.kpi{background:var(--panel);border:1px solid var(--line);border-radius:15px;padding:17px;box-shadow:var(--shadow);min-width:0}
...
<main class="page" id="page">
  <div class="heading-row">
    <div><h1>Service / SLA</h1><div class="subtitle">Queue responsiveness, SLA attainment, abandon pressure and long-wait risk.</div></div>
    <div style="display:flex;align-items:center;gap:10px"><button class="info-btn" id="infoBtn">ⓘ Metric Definitions</button><div class="subtitle" id="version">Loading backend…</div></div>
  </div>
  ...
  <section class="filters">
    <div class="field"><label>From</label><input id="fromDate" type="date"></div>
    <div class="field"><label>Through</label><input id="toDate" type="date"></div>
    <div class="field"><label>SLA Target (sec)</label><input id="slaSeconds" type="number" min="0" value="60"></div>
    <div class="field"><label>Long Wait (sec)</label><input id="longWaitSeconds" type="number" min="0" value="300"></div>
    <div class="field"><label>Short Abandon (sec)</label><input id="shortAbandonSeconds" type="number" min="0" value="0"></div>
    <button class="apply" id="applyBtn">Apply</button>
  </section>
  ...
  <section class="kpis">
    ...
    <div class="kpi"><div class="kpi-label">Strict SLA</div><div class="kpi-value" id="strictSla">—</div><div class="kpi-foot">within target ÷ queued</div></div>
    <div class="kpi"><div class="kpi-label">Answered Within SLA</div><div class="kpi-value" id="answeredSla">—</div><div class="kpi-foot">within target ÷ answered</div></div>
    ...
    <div class="kpi"><div class="kpi-label">Avg Wait</div><div class="kpi-value" id="avgWait">—</div><div class="kpi-foot">all queued inbound</div></div>
    <div class="kpi"><div class="kpi-label">Max Wait</div><div class="kpi-value" id="maxWait">—</div><div class="kpi-foot">longest queue wait</div></div>
    <div class="kpi"><div class="kpi-label">Long-Wait Breaches</div><div class="kpi-value" id="longBreaches">—</div><div class="kpi-foot" id="longBreachFoot">—</div></div>
    <div class="kpi"><div class="kpi-label">Answered Over SLA</div><div class="kpi-value" id="overSla">—</div><div class="kpi-foot">answered after target</div></div>
  </section>
    ...
    <div class="queue-wrap">
      <table>
        <thead><tr>
          ...
          <th data-sort="answered">Answered</th>
          <th data-sort="abandoned">Abandoned</th>
          <th data-sort="strict_sla_percent">Strict SLA</th>
          <th data-sort="answered_within_sla_percent">Answered SLA</th>
          <th data-sort="abandon_rate">Abandon %</th>
          <th data-sort="avg_wait_seconds">Avg Wait</th>
          <th data-sort="max_wait_seconds">Max Wait</th>
          <th data-sort="long_wait_breach_percent">Long-Wait %</th>
        </tr></thead>
        <tbody id="queueBody"></tbody>
      </table>
    </div>
    <div class="note">Thresholds are configurable above. This dashboard does not assume these formulas exactly match a customer’s Cisco Analyzer SLA configuration.</div>
  </section>
</main>
...
<div class="modal-backdrop" id="modal">
  <div class="modal-card">
    ...
    <div class="defs">
      ...
      <div class="def"><strong>Strict SLA %</strong><p>Calls answered within the SLA target divided by all queued inbound calls.</p><span class="formula">Answered Within SLA ÷ Queued Inbound</span></div>
      <div class="def"><strong>Answered Within SLA %</strong><p>Of the calls that were eventually answered, the percentage answered within the target.</p><span class="formula">Answered Within SLA ÷ Answered</span></div>
      ...
      <div class="def"><strong>Long-Wait Breach</strong><p>A queued interaction whose queue wait exceeds the configured Long Wait threshold.</p></div>
      <div class="def"><strong>Average / Maximum Wait</strong><p>Queue-duration measurements from the task-level interaction record.</p></div>
    </div>
  </div>
</div>
...
function apiUrl(){
  ...
  p.set('sla_seconds',$('slaSeconds').value||60);
  p.set('long_wait_seconds',$('longWaitSeconds').value||300);
  p.set('short_abandon_seconds',$('shortAbandonSeconds').value||0);
...
function renderOverview(o){
  ...
  $('longBreachFoot').textContent=pct(o.long_wait_breach_percent)+' of queued';
...
function renderHours(rows){
  ...
  $('hourGrid').innerHTML=active.length?active.map(r=>`<div class="hour-card ${classForSla(r.strict_sla_percent)}">
    ...
    <span>${pct(r.strict_sla_percent)} strict SLA</span>
...
function renderInsights(o){
  const items=[
    ['Worst SLA Queue',o.worst_sla_queue?`${o.worst_sla_queue.queue_name} · ${pct(o.worst_sla_queue.strict_sla_percent)}`:'—'],
    ['Highest Abandon Queue',o.highest_abandon_queue?`${o.highest_abandon_queue.queue_name} · ${pct(o.highest_abandon_queue.abandon_rate)}`:'—'],
    ['Longest Wait Queue',o.longest_wait_queue?`${o.longest_wait_queue.queue_name} · ${wait(o.longest_wait_queue.max_wait_seconds)}`:'—'],
    ['Answered Within Target',`${o.answered_within_sla} of ${o.answered} answered calls`],
    ['Answered Over Target',`${o.answered_over_sla} calls`],
    ['Abandoned Over Target',`${o.abandoned_over_sla} calls`],
  ];
