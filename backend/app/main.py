...
def home():
    ...
    <body>
      <main class="shell">
        ...
        <div class="status">Dashboard backend v5.2</div>
      </main>
    </body>
    </html>
    """
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--ink)}
    ...
    h1{margin:0;font-size:30px;letter-spacing:-.025em}.subtitle{margin-top:5px;color:var(--muted);font-size:14px}
    .filters{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:16px;display:grid;grid-template-columns:repeat(4,minmax(130px,1fr)) auto;gap:12px;align-items:end;box-shadow:var(--shadow);margin-bottom:18px}
...
<main class="page" id="page">
  <div class="heading-row">
    <div><h1>Service / SLA</h1><div class="subtitle">Queue responsiveness, SLA attainment against a fixed 15-second answer target, abandon pressure and long-wait risk.</div></div>
    <div style="display:flex;align-items:center;gap:10px"><button class="info-btn" id="infoBtn">ⓘ Metric Definitions</button><div class="subtitle" id="version">Loading backend…</div></div>
  </div>
...
<div class="modal-backdrop" id="modal">
  <div class="modal-card">
    ...
    <div class="defs">
      ...
      <div class="def"><strong>Strict SLA %</strong><p>Calls answered within the fixed 15-second SLA target divided by all queued inbound calls.</p><span class="formula">Answered Within 15s ÷ Queued Inbound</span></div>
      <div class="def"><strong>Answered Within SLA %</strong><p>Of the calls that were eventually answered, the percentage answered within 15 seconds.</p><span class="formula">Answered Within 15s ÷ Answered</span></div>
