...
def home():
    ...
    <body>
      <main class="shell">
        ...
        <div class="status">Dashboard backend v6.4</div>
      </main>
    </body>
    </html>
    """
...
<main class="page">
  <div class="heading-row">
    <div><h1>Missed Calls & Callbacks</h1><div class="subtitle">Track missed queue calls, native courtesy callback resolution, and follow-up risk.</div><div class="policy">Unresolved aging threshold: 24 hours · Native courtesy callbacks only</div></div>
  ...
  <section class="kpis">
    ...
    <div class="kpi"><div class="kpi-label">Resolved by Callback</div><div id="resolved" class="kpi-value">—</div><div class="kpi-foot">matched successful native callback</div></div>
    <div class="kpi"><div class="kpi-label">Callback Resolution Rate</div><div id="resolutionRate" class="kpi-value">—</div><div class="kpi-foot">resolved ÷ missed</div></div>
    <div class="kpi"><div class="kpi-label">Unresolved Missed Calls</div><div id="unresolved" class="kpi-value">—</div><div class="kpi-foot">no successful matched callback</div></div>
    <div class="kpi"><div class="kpi-label">Unresolved Over 24h</div><div id="over24" class="kpi-value">—</div><div class="kpi-foot">follow-up risk</div></div>
    <div class="kpi"><div class="kpi-label">Avg Time to Callback</div><div id="avgCallback" class="kpi-value">—</div><div class="kpi-foot">missed call to successful callback</div></div>
  </section>
  ...
  <section class="panel">
    <div class="panel-head"><div><div class="panel-title">Queue Follow-Up Performance</div><div class="panel-meta">Missed calls and native callback resolution by queue</div></div></div>
    <div class="table-wrap"><table>
      <thead><tr><th>Queue</th><th>Queued</th><th>Missed</th><th>Missed Rate</th><th>Resolved by Callback</th><th>Unresolved</th><th>Resolution Rate</th></tr></thead>
      <tbody id="queueRows"></tbody>
    </table></div>
    <div class="note">“Resolved by Callback” only counts a successful native WxCC courtesy callback matched to the same caller after the missed call. Manual outbound calls are not included yet.</div>
  </section>

  <section class="panel" style="margin-top:18px">
    <div class="panel-head"><div><div class="panel-title">Resolved Callback Detail</div><div class="panel-meta">Validated native callback matches</div></div></div>
    <div class="table-wrap"><table>
...
<div id="defs" class="modal" onclick="if(event.target===this)closeDefs()"><div class="modal-card">
  ...
  <div class="def"><strong>Resolved by Callback</strong><p>A missed caller whose number was later matched to a successful native WxCC courtesy callback.</p></div>
  <div class="def"><strong>Callback Resolution Rate</strong><p>The percentage of missed calls that were resolved by a matched successful native callback.</p></div>
...
function renderSummary(o){
  const rows=[
    ...
    ['Unresolved Missed Calls',`${o.unresolved}`],
    ['Unresolved Over 24 Hours',`${o.unresolved_over_threshold}`],
    ['Successful Native Callback Records',`${o.successful_native_callback_records}`],
    ['Avg Time to Callback',duration(o.avg_time_to_native_callback_seconds)]
  ];
...
function renderQueues(rows){
  $('queueRows').innerHTML=rows.map(r=>`<tr>
    ...
    <td>${r.resolved_by_native_callback}</td><td>${r.unresolved}</td><td>${pct(r.native_resolution_rate)}</td>
  </tr>`).join('')||'<tr><td colspan="7" class="empty">No queue data.</td></tr>';
}
...
async function loadData(){
  try{
    ...
    $('unresolved').textContent=o.unresolved;
    $('over24').textContent=o.unresolved_over_threshold;
