...
def dashboard_health_summary(db: Session):
    """
    Operational health summary based only on stored analytics data timestamps.
    This does not inspect Render process health or external WxCC availability.
    """
    newest_interaction_ms = db.query(func.max(Interaction.created_time)).scalar()
    newest_session_ms = db.query(func.max(AgentSession.created_time)).scalar()
    newest_state_ms = db.query(func.max(AgentStateActivity.start_time)).scalar()

    timestamps = [
        ts for ts in [newest_interaction_ms, newest_session_ms, newest_state_ms]
        if ts is not None
    ]
    newest_data_ms = max(timestamps) if timestamps else None

    now_ms = int(datetime.now(tz=timezone.utc).timestamp() * 1000)

    age_minutes = None
    if newest_data_ms is not None:
        age_minutes = max(0, round((now_ms - int(newest_data_ms)) / 60000, 1))

    # Freshness is intentionally simple and operational:
    # healthy <= 30m, delayed <= 90m, stale > 90m.
    if newest_data_ms is None:
        status = "no_data"
    elif age_minutes <= 30:
        status = "healthy"
    elif age_minutes <= 90:
    ...
    return {
        ...
        "note": (
            "Status reflects freshness of stored analytics data only. "
            "It does not verify Render process uptime or live WxCC API availability."
        ),
    }
