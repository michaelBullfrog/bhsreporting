...
def home():
    ...
    <body>
      <main class="shell">
        ...
        <div class="status">Dashboard backend v7.1</div>
      </main>
    </body>
    </html>
    """
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Inbound / Outbound | WxCC Analytics</title>
<style>
:root{--bg:#f3f6f5;--panel:#fff;--ink:#17221d;--muted:#69766f;--line:#dde6e1;--brand:#1f7a4c;--brand-dark:#145c38;--soft:#e9f5ee;--bad:#b42318;--badbg:#fff0ee;--warn:#9a6700;--warnbg:#fff7dd;--shadow:0 7px 24px rgba(20,55,39,.07)}
*{box-sizing:border-box}body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--ink)}
button,input{font:inherit}.topbar{height:66px;display:flex;align-items:center;justify-content:space-between;padding:0 28px;background:#173d2b;color:#fff;position:sticky;top:0;z-index:10}
...
<div class="topbar">
  <div class="brand"><div class="mark">B</div><div><div class="brand-title">Bullfrog Analytics</div><div class="brand-sub">Webex Contact Center</div></div></div>
  <nav class="nav"><a href="/">Home</a><a href="/staffing">Staffing</a><a href="/call-demand">Call Demand</a><a href="/service-sla">Service / SLA</a><a href="/missed-callbacks">Missed & Callback</a><a class="active" href="/inbound-outbound">Inbound / Outbound</a></nav>
</div>
<main class="page">
  <div class="heading-row">
    <div><h1>Inbound / Outbound</h1><div class="subtitle">Compare call mix, agent outbound activity, and inbound performance during outbound-heavy periods.</div><div class="policy">15-second inbound SLA · Task-level call counts · Correlation only, not causation</div></div>
    <button class="btn" style="background:#fff;color:var(--brand-dark);border:1px solid var(--line)" onclick="openDefs()">Metric Definitions</button>
  </div>
  ...
  <section class="kpis">
    ...
    <div class="kpi"><div class="kpi-label">15-Second Inbound SLA</div><div id="sla" class="kpi-value">—</div><div class="kpi-foot">queued calls answered in 15s</div></div>
    <div class="kpi"><div class="kpi-label">Inbound Missed Rate</div><div id="missedRate" class="kpi-value">—</div><div class="kpi-foot">missed ÷ queued inbound</div></div>
    <div class="kpi"><div class="kpi-label">Outbound Connect Rate</div><div id="connectRate" class="kpi-value">—</div><div class="kpi-foot">connected ÷ outbound calls</div></div>
  ...
  <section class="grid">
    <div class="panel"><div class="panel-head"><div><div class="panel-title">Hourly Call Mix</div><div class="panel-meta">Inbound vs outbound task counts</div></div></div><div class="panel-body"><div id="bars" class="bars"></div></div></div>
    <div class="panel"><div class="panel-head"><div><div class="panel-title">Outbound Activity vs Inbound Performance</div><div class="panel-meta">Descriptive comparison</div></div></div><div class="panel-body">
      <div id="compare" class="compare"></div>
      <div class="warning">This comparison shows whether inbound metrics were different during hours with more outbound calling. It does not prove outbound activity caused the SLA or missed-rate difference.</div>
    </div></div>
  </section>
  ...
  <section class="panel" style="margin-top:18px">
    ...
    <div class="table-wrap"><table>
      <thead><tr><th>Hour</th><th>Inbound</th><th>Queued Inbound</th><th>Outbound</th><th>Outbound Share</th><th>15s SLA</th><th>Inbound Missed</th><th>Missed Rate</th><th>Outbound Talk</th></tr></thead>
...
<div id="defs" class="modal" onclick="if(event.target===this)closeDefs()"><div class="modal-card">
  ...
  <div class="def"><strong>15-Second Inbound SLA</strong><p>The percentage of queued inbound calls answered within 15 seconds.</p></div>
  <div class="def"><strong>Inbound Missed Rate</strong><p>The percentage of queued inbound calls that never connected to an agent.</p></div>
  <div class="def"><strong>Outbound Connect Rate</strong><p>The percentage of outbound interactions that connected.</p></div>
  <div class="def"><strong>Outbound Talk Time</strong><p>Total connected duration on outbound interactions.</p></div>
  <div class="def"><strong>Higher vs Lower Outbound Activity</strong><p>Hours are split around the median outbound calls per active outbound hour. The comparison is descriptive and does not establish causation.</p></div>
</div></div>
...
function compareCard(title,r){
  return `<div class="compare-card"><h3>${title}</h3>
    <div class="metric"><span>Hours</span><strong>${r.hours}</strong></div>
    <div class="metric"><span>Outbound Calls</span><strong>${r.outbound_calls}</strong></div>
    <div class="metric"><span>Queued Inbound</span><strong>${r.queued_inbound}</strong></div>
    <div class="metric"><span>15-Second SLA</span><strong>${pct(r.inbound_15s_sla)}</strong></div>
    <div class="metric"><span>Inbound Missed Rate</span><strong>${pct(r.inbound_missed_rate)}</strong></div>
  </div>`;
}
function renderCompare(c){
  $('compare').innerHTML=compareCard('Higher Outbound Activity',c.high_outbound_hours)+compareCard('Lower Outbound Activity',c.lower_outbound_hours);
}
...
function renderHours(rows){
  $('hourRows').innerHTML=rows.map(r=>`<tr>
    ...
    <td>${pct(r.outbound_share)}</td><td>${pct(r.inbound_15s_sla)}</td><td>${r.inbound_missed}</td>
    <td>${pct(r.inbound_missed_rate)}</td><td>${duration(r.outbound_talk_seconds)}</td>
  </tr>`).join('')||'<tr><td colspan="9" class="empty">No hourly activity.</td></tr>';
}
async function loadData(){
  try{
    const res=await fetch(apiUrl());if(!res.ok)throw new Error(`${res.status} ${res.statusText}`);
    ...
    $('sla').textContent=pct(o.inbound_15s_sla);
    $('missedRate').textContent=pct(o.inbound_missed_rate);
    $('connectRate').textContent=pct(o.outbound_connect_rate);
    $('talk').textContent=duration(o.outbound_talk_seconds);
    renderBars(d.hourly||[]);renderCompare(d.comparison);renderAgents(d.agents||[]);renderHours(d.hourly||[]);
  }catch(e){$('compare').innerHTML=`<div class="error">Could not load inbound/outbound data: ${e.message}</div>`}
}
setDefaults();loadData();
</script>
</body></html>
